...
export default async function cli ( dc: string | undefined, tw: string | undefined, irc: string | undefined, out: string | undefined, { zip, d, t, i, o }: cliOptions ) {
	...
	await analyze(discordPath, twitchPath, ircPath, outPath, zip)
	await cleanupZip()
}

async function analyze ( discordZip: string | null, twitchDir: string | null, ircTxt: string | null, outDir: string, zip: boolean ) {
